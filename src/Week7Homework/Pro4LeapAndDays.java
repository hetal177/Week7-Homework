...
*/
    ...
    static int year; // static variavle as both method is static
    static int month;

    public static void main(String[] args) {
        Scanner input = new Scanner(System.in); // used for consol input
        System.out.println("enter any year");
        year = input.nextInt();
        System.out.println("enter  month");
        month = input.nextInt();
        getDaysInMonth(month, year); // direct method calling
        isLeapYear(year);// direct method calling
        input.close();
    }


    public static boolean isLeapYear(int year) {
        if (year % 4 != 0) { // if .. else if  .. else used
            return false;
        } else if (year % 400 == 0) {
            return true;
        } else if (year % 100 == 0) {
    ...
    public static void getDaysInMonth(int month, int year) {
        ...
        if ((month < 1) || (month > 12) || (year < 1) || (year > 9999)) { // only if used to check first condition
            System.out.println("-1");

        }
        switch (month) { // swith used as  as onr out put per case except case 2
            case 1: // also created case culsters one has same out put
            case 3:
            case 5:
            case 7:
            case 8:
            ...
            case 12:
                ...
                break;
            case 2: // so called method isLeapyear through class and if else for two different output
